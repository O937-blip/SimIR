...
def SoftLoULoss(pred, target,num_masks):
        target = target.float()
        pred = F.sigmoid(pred)
        smooth = 0.00
        ...
        l1 = nn.BCELoss()
        loss1 = l1(pred, target)
        losses = {"loss_mask_dice_0":loss + loss1}
...
class SetCriterion(nn.Module):
    ...
    def sirst_loss(self, outputs, targets, num_masks):
        """Compute the losses related to the masks: the focal loss and the dice loss.
        targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """

        src_masks = outputs['predictions_mask']
        ...
        #loss = SoftIoULoss(src_masks,target_masks)
        losses = {
            "loss_mask_bce_0": sigmoid_ce_loss_jit(point_logits, point_labels, num_masks),
            "loss_mask_dice_0": dice_loss_jit(point_logits, point_labels, num_masks),
        }
        losses["loss_mask_bce_0"] = losses["loss_mask_bce_0"].sum() / b

        dice_loss_all = losses["loss_mask_dice_0"]
        losses["loss_mask_dice_0"] = dice_loss_all.sum() / b

        #losses["loss_mask_focal_0"] = sigmoid_focal_loss(point_logits,point_labels).sum() /b

        if "iou_prediction" in outputs:
            src_ious = outputs["iou_prediction"]
            src_ious = src_ious.flatten(0)

            target_iou = 1 - dice_loss_all
            losses['iou_score_loss_0'] = iou_score_loss(src_ious, target_iou).sum() / num_masks
        #l$osses["iou_score_loss_0"] = loss
        del src_masks
        del target_masks
        return losses
    ...
    def forward(self, outputs, targets, mask_dict=None, task='sam', extra={}):
        ...
        if task=='sam':
            ...
            if mask_dict is None:
                sudo_loss = sum([outputs[key].sum() for key in outputs.keys() if
                                 key != 'aux_outputs' and outputs[key] is not None]) * 0.0
                losses = dict()
                l_dict = dict()
            ...
            for i, output in enumerate(outputs):
                l_dict = self.sirst_loss(output, tgt, num_masks=1)
                l_dict = {k.replace('_0', f"_{i}"): v for k, v in l_dict.items()}
                losses.update(l_dict)
