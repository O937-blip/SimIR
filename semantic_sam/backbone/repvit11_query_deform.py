...
from ..body.encoder.encoder_deform import *
...
class RepViT(nn.Module):
    def __init__(self, cfgs, lang, num_classes=1000, distillation=False):
        ...
        self.broad_256 = torch.nn.Sequential(Conv2d_BN(64, 256, 1, 1, 0), torch.nn.GELU())
        self.broad_128 = torch.nn.Sequential(Conv2d_BN(128, 256, 1, 1, 0), torch.nn.GELU())
        self.broad_64 = torch.nn.Sequential(Conv2d_BN(256, 256, 1, 1, 0), torch.nn.GELU())
        self.broad_32 = torch.nn.Sequential(Conv2d_BN(512, 256, 1, 1, 0), torch.nn.GELU())
        ...
        self.proj_32 = torch.nn.Sequential(Conv2d_BN(256, 512, 1, 1, 0), torch.nn.GELU())

        self.proj_256_dense = torch.nn.Sequential(Conv2d_BN(64, 8, 1, 1, 0), torch.nn.GELU())
        self.proj_128_dense = torch.nn.Sequential(Conv2d_BN(128, 8, 1, 1, 0), torch.nn.GELU())
        self.proj_64_dense = torch.nn.Sequential(Conv2d_BN(256, 8, 1, 1, 0), torch.nn.GELU())
        self.proj_32_dense = torch.nn.Sequential(Conv2d_BN(512, 8, 1, 1, 0), torch.nn.GELU())

        self.broad_128_dense = torch.nn.Sequential(Conv2d_BN(8, 128, 1, 1, 0), torch.nn.GELU())
        self.broad_64_dense = torch.nn.Sequential(Conv2d_BN(8, 256, 1, 1, 0), torch.nn.GELU())
        self.broad_32_dense = torch.nn.Sequential(Conv2d_BN(8, 512, 1, 1, 0), torch.nn.GELU())
        self.n = 0
        ...
        self.query_feat_encoder = nn.Embedding(4, 256)
        self.query_embed_encoder = nn.Embedding(4, 256)
        ...
        self.transformer1 = MSDeformAttnTransformerEncoderOnly(
            d_model=8,
            dropout=0,
            nhead=8,
            dim_feedforward=2048,
        ...
        self.transformer2 = MSDeformAttnTransformerEncoderOnly(
            d_model=8,
            dropout=0,
            nhead=8,
            dim_feedforward=2048,
        ...
        self.transformer3 = MSDeformAttnTransformerEncoderOnly(
            d_model=8,
            dropout=0,
            nhead=8,
            dim_feedforward=2048,
        ...
        self.pe_layer2 = PositionEmbeddingSine(4, normalize=True)
    ...
    def forward(self, x):
        ...
        for i, f in enumerate(self.features):
            ...
            if i == 4:
                ...
                d2 = self.proj_128_dense(x)
                d2_pos = self.pe_layer2(d2)
                list = []
                list.append(d1)
                list.append(d2)
                pos = []
                pos.append(d1_pos)
                pos.append(d2_pos)
                y, spatial_shapes, level_start_index = self.transformer1(list, None, pos, use_ckpt=False)
                y = torch.split(y, level_start_index[1], dim=1)
            ...
            if i == 8:
                ...
                pos.append(d1_pos)
                pos.append(d2_pos)
                y, spatial_shapes, level_start_index = self.transformer2(list, None, pos, use_ckpt=False)
                y = torch.split(y, level_start_index[1], dim=1)
            ...
            if i == 22:
                ...
                pos.append(d1_pos)
                pos.append(d2_pos)
                y, spatial_shapes, level_start_index = self.transformer3(list, None, pos, use_ckpt=False)
                y = torch.split(y, level_start_index[1], dim=1)
        ...
        mask_features = self.mask_features(dense_256)
