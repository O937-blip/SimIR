...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.proj2 = nn.Conv2d(in_chans, embed_dim, kernel_size=3, stride=2,padding=1)
...
class D2SwinTransformer(SwinTransformer, Backbone):
    ...
    def forward(self, x):
        ...
        for i in range(b):
            tensor = torch.mean(outputs['res2'][i, :, :, :], dim=0)
            ...
            plt.savefig(f'/home/zc/IRR/visual/irstd/swinl_notrain/res2/heatmap_{self.n}.png')
            plt.close()
        b, c, h, w = outputs['res3'].shape
        for i in range(b):
            tensor = torch.mean(outputs['res3'][i, :, :, :], dim=0)
            tensor = (tensor - torch.min(tensor)) / (torch.max(tensor) - torch.min(tensor))
            tensor = tensor.cpu().detach().numpy()
            plt.imshow(tensor, cmap='hot', interpolation='nearest')
            plt.colorbar()
            ...
            plt.savefig(f'/home/zc/IRR/visual/irstd/swinl_notrain/res3/heatmap_{self.n}.png')
            plt.close()
        b, c, h, w = outputs['res4'].shape
        for i in range(b):
            tensor = torch.mean(outputs['res4'][i, :, :, :], dim=0)
            tensor = (tensor - torch.min(tensor)) / (torch.max(tensor) - torch.min(tensor))
            tensor = tensor.cpu().detach().numpy()
            plt.imshow(tensor, cmap='hot', interpolation='nearest')
            plt.colorbar()
            ...
            plt.savefig(f'/home/zc/IRR/visual/irstd/swinl_notrain/res4/heatmap_{self.n}.png')
            plt.close()
        b, c, h, w = outputs['res5'].shape
        for i in range(b):
            tensor = torch.mean(outputs['res5'][i, :, :, :], dim=0)
            tensor = (tensor - torch.min(tensor)) / (torch.max(tensor) - torch.min(tensor))
            tensor = tensor.cpu().detach().numpy()
            plt.imshow(tensor, cmap='hot', interpolation='nearest')
            plt.colorbar()
            ...
            plt.savefig(f'/home/zc/IRR/visual/irstd/swinl_notrain/res5/heatmap_{self.n}.png')
            plt.close()
        self.n = self.n+1'''
        return outputs
