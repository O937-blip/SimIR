...
class ARCH_SIRST(nn.Module):
    ...
    def forward_seg(self, batched_inputs, task='sirst',
        ...
        for i in range(b):
            ...
            image.save(f'/home/imglab/zc/irr/visual/irdst/gt/og_{self.n}.png')'''

        # image = to_pil_image(tensor_normalized)

        # 保存图像为PNG文件
        # image.save('path/to/save/visualization.png')
        ...
        if self.training:
            ...
            if task == 'sirst':
                ...
                for i in range(b):
                    ...
                    slice = (slice>0).float()
                    image = to_pil_image(slice)
                    image.save(f'/home/imglab/zc/irr/visual/irdst/gt_mask/ogt_{self.n}.png')'''
    ...
    def evaluate_sirst(self, batched_inputs, task='seg',
        ...
        for i in range(b):
            ...
            slice = (slice - torch.min(slice)) / (torch.max(slice) - torch.min(slice))
            image = to_pil_image(slice)
            image.save(f'/home/zmj/zc/irr/visual/irstd/gt/og_{self.n}.png')'''
        features,outputs_mask,sparse_query_encoder = self.backbone(images.tensor)
        ...
        for i in range(b):
            ...
            slice = (slice > 0).float()
            image = to_pil_image(slice)
            image.save(f'/home/zmj/zc/irr/visual/irstd/gt_mask/ogt_{self.n}.png')'''
        ...
        for i in range(b):
            ...
            slice = (slice > 0).float()
            image = to_pil_image(slice)
            image.save(f'/home/zmj/zc/irr/visual/momov16/output_mask/out_{self.n}.png')'''
        result = {
