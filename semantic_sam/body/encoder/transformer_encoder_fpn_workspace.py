    ...
    def forward_features(self, features):
        ...
        for idx, f in enumerate(self.in_features[::-1]):
            ...
            if lateral_conv is None:
                ...
                src = transformer.flatten(2) + self.level_embed_proj.weight[num_cur_levels][None, :, None]
                pos = pos.permute(2, 0, 1)
                src = src.permute(2, 0, 1)
                _, bs, _ = src.shape
                query_feat_fpn, query_embed_fpn = self.lang.get_fpn()
                query_embed_fpn = query_embed_fpn.weight.unsqueeze(1).repeat(1, bs, 1)
                query_feat_fpn = query_feat_fpn.weight.unsqueeze(1).repeat(1, bs, 1)

                query_feat_decoder, query_embed_decoder = self.lang.get_decoder()
                query_embed_decoder = query_embed_decoder.weight.unsqueeze(1).repeat(1, bs, 1)
                query_feat_decoder = query_feat_decoder.weight.unsqueeze(1).repeat(1, bs, 1)


                query_embed = torch.cat([query_embed_fpn,query_embed_decoder],dim=0)
                query_feat = torch.cat([query_feat_fpn, query_feat_decoder], dim=0)
...
@register_encoder
def get_transformer_encoder_fpn_workspace(cfg, input_shape,lang):
    """
