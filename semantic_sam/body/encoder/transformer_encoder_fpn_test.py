...
class TransformerEncoderPixelDecoder(BasePixelDecoder):
    ...
    def forward_features(self, features,sparse_query_encoder):
        multi_scale_features = []
        num_cur_levels = 0
        # QxNxC
        ...
        for idx, f in enumerate(self.in_features[::-1]):
            ...
            if lateral_conv is None:
                ...
                _, bs, _ = src.shape
                query_embed_encoder = self.query_embed_encoder.weight.unsqueeze(1).repeat(1, bs, 1)
                query_feat_encoder = sparse_query_encoder

                query_embed = self.query_embed.weight.unsqueeze(1).repeat(1, bs, 1)
                query_embed = torch.cat([query_embed,query_embed_encoder],dim=0)
                query_feat = self.query_feat.weight.unsqueeze(1).repeat(1, bs, 1)
                query_feat = torch.cat([query_feat, query_feat_encoder], dim=0)
        ...
        self.n = self.n+1
        return mask_features, outputs_mask, multi_scale_features,query_feat
