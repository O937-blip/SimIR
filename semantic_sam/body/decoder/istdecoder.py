...
class SEEMDecoder(nn.Module):
    ...
    def forward(self, x, mask_features,dense_embeddings, sparse_query_fpn, target_queries=None, target_vlp=None, task='seg', extra={}):
        # x is a list of multi-scale feature
        assert len(x) == self.num_feature_levels;
        mask_features = mask_features+dense_embeddings
        src, pos, size_list = self.prepare_features(x, self.num_feature_levels, self.pe_layer, self.input_proj,
                                                    self.level_embed)

        _, bs, _ = src[0].shape
        query_feat,query_embed = self.query_feat,self.query_embed
        sparse_embed = self.sparse_embed
        sparse_embed = sparse_embed.weight.unsqueeze(1).repeat(1, bs, 1)
        # QxNxC
        query_embed = query_embed.weight.unsqueeze(1).repeat(1, bs, 1)
        query_embed = torch.cat([query_embed,sparse_embed],dim=0)
        output = query_feat.weight.unsqueeze(1).repeat(1, bs, 1)
        output = torch.cat([output,sparse_query_fpn],dim=0)
        ...
        for i in range(self.num_layers):
            results[-1]["attn_mask"][torch.where(results[-1]["attn_mask"].sum(-1) == results[-1]["attn_mask"].shape[-1])] = False
            level_index = i % (self.num_feature_levels)
            # CROSS ATTENTION
            output, avg_attn = self.transformer_cross_attention_layers[i](
                output, src[level_index],memory_mask=results[-1]["attn_mask"],
                pos=pos[level_index], query_pos=query_embed
            )


            # SELF ATTENTION
            self_attn_mask = torch.zeros((bs, self.num_queries, self.num_queries),
                                         device=query_embed.device).bool()  # Default False (attend oq)
            ...
            results.append(self.forward_prediction_heads(output, mask_features,
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, layer_id=-1):
        decoder_output = self.decoder_norm(output[0].unsqueeze(0))
        ...
        attn_mask = (attn_mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 9, 1).flatten(0,
                                                                                                         1) < 0.5).bool()
