...
from ...mlp_esam import MLPBlock
from ...two_way_transformer_esam import TwoWayTransformer
...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        masks, iou_pred,res4 = self.predict_masks(
            image_embeddings=image_embeddings,
            image_pe=image_pe,
            sparse_prompt_embeddings=sparse_prompt_embeddings,
        )
        if multimask_output and self.num_multimask_outputs > 1:
            return masks[:, 1:, :], iou_pred[:, 1:],res4
        else:
            return masks[:, :1, :], iou_pred[:, :1],res4
    ...
    def predict_masks(
        ...
        sparse_prompt_embeddings = sparse_prompt_embeddings.repeat(image_embeddings.size(0),1,1)
        ...
        return masks, iou_pred,short

from .registry import register_decoder
@register_decoder
def sam_mask_decoder(cfg, in_channels, lang_encoder, mask_classification, extra):
    img_size = 1024
    ...
    encoder_transformer_output_dim = 256
    return MaskDecoder(
